...
离线起一个服务 + 单测纯函数，共 **412 项**，应全绿。不联网、不碰真机、不写你的桌面。
...
### 「机器运行实况」的 8 列固定排版（v1.5.5）

这张表有 8 列，却塞在 `span-7` 卡片里，用浏览器默认的 `table-layout: auto` 有两个**实测**出来的硬伤：

| 现象 | 实测数据 |
| --- | --- |
| ① 列宽随内容 / 行数抖动 | 同一张表，2 行时「最后在线」列 **31.5px**，48 行时 **77px** —— 列宽不该由数据量决定 |
| ② 内容顶破卡片 → 横向滚动条 | 溢出 **37px**（2 行）~ **83px**（48 行），卡片底部凭空多出一条横向滚动条 |

**修法**：列宽不再交给浏览器算，改成 `table-layout: fixed` + `index.html` 里的 `<colgroup>`
（`c-host / c-ip / c-state / c-role / c-restore / c-snap / c-seen / c-ops` 八个 `<col>`），
每列宽度按「**内容自然宽 + 18px 内边距**」的实测需求写死，合计 ≈945px：

```
主机 149 / IP 118 / 状态 108 / 角色 90 / 恢复 103 / 快照 109 / 最后在线 102 / 操作 154
```

超长内容一律**就地换行**（`.uptime`、`.ops-cell` 都从 `nowrap` 改回 `normal`），不再靠撑宽列来解决。
配套的「视觉协调」细节：

- **顶端对齐** `vertical-align: top` —— 各列行数不同（主机 2 行 / 快照 2 行 / IP 1 行），居中对齐会让每列基线互相错位。
- **行高下限** `tbody tr { height: 54px }` —— 单行行（离线机器）别和双行行（在线机器）差出一倍高度。
- **斑马纹 + 悬停底色** —— 8 列、行高又不齐，没有底色引导很容易串行。
- **安静占位** `.none` —— 「—」「离线」这类空值收成小灰字，别跟真内容抢注意力。
- **概览行独占一行** `.card-sub` —— 「在线 2 / 共 48 个节点」以前塞在卡片头右侧，机器一多就把开关/按钮挤到换行。
- **快照列拆两行** —— 时间徽章在上、`☁ 一键备份` 按钮在下；**文件数移进 tooltip**。
  原先「时间 · 文件数」挤在一个徽章里，自然宽 163px，是整张表最宽的一列、也是横向滚动条的主要来源；
  拆开后该列需求从 183px 降到 98px。
- **控件收内边距** —— `.btn-mini / .badge / .btn-backup` 在机器表里各收 1~2px。8 列的内容总宽是硬需求，只能从内边距里省。

**窄视口回退**：八列要 ≈945px 才排得开，而 `span-7` 卡片只有在视口 ≳1700px 时才够宽
（实测 1680 视口卡片仅 **934.5px** → 快照列溢出 3px；1560 视口溢出 11px）。
所以 `@media (max-width: 1720px)` 时让机器表与账号卡**各占整行**（`grid-column: span 12`）——
与其让表格挤成横向滚动条 / 单元格内容顶破列边框，不如给它整行宽度。阈值取 1720（而非 1700）是留 12px 余量。

> 这些数字不是拍脑袋来的：用无头 Edge（`--headless=new --remote-debugging-port`）+ CDP `Runtime.evaluate`
